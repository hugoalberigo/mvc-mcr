const {match, rooms, round, user_game_history, users} = require("../models")
...
exports.fight = async (req, res, next) => {
    ...
    if (active_round <= 3) {
                ...
                if (winner_id !== null || winner_id !== undefined) {
                    ...
                    if (winner_id == 0) {
                        return res.status(200).json({
                            message: "Tie, result is DRAW!",
                            data: match_data
                        })
                    } else {
                        return res.status(200).json({
                            message: "User " + winner_id + " WIN!",
                            data: match_data
                        })
                    }
                } else {
        ...
        var winner_username = await users.get_username(winner_of_this_room)

        await user_game_history.score_record(winner_of_this_room, winner_username)
